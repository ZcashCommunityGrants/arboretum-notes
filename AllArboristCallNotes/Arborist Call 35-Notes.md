...
Then full DAGSync beyond that will be a more sizeable change to the interface that SDK users use. We are still figuring out how the API surface will change. That will come after that. 
...
Quick question on that. The update that's coming next week or soonish, will that have parallel trial decryption? 
...
The other thing is we are really advancing with implementation, we are more or less halfway with the circuit. As of now we may be exceeding some of the limits that we hoped to achieve in terms of circuit size. It may be that we need to rethink some of the constraints. At some point when we finish with most of the implementation we should have a talk with St4d & Daira to figure out any optimisations in terms of space. Also based on the talk that St4d gave at Zcon3 that was really inspiring. 
...
It was just a comment that was raised somewhere to do with the fact that bridging ethereum in, the way the ETH token is defined, it uses a unit of account that ends up being larger than uint64 can hold. So to be able to represent wei in ZSA we would need to represent notes with larger than uint64 values. 

[42:18]- **Daniel**

We discussed it at one point, we will make sure its on the top of the list. 
...
I'll make an analogy here. If you're familiar with setting up SSH keys, you generally don't want to copy your private SSH keys off of your machine and put them on another machine. Instead you want to go onto the other machine and generate another set of keys and distribute the public keys for those so that you are never moving that secret information in a way that could be captured. 

[51:16] - **Str4d**

The affordance relaxed here within the accounts model is the mnemonic seed phrase is the backup for when you need to load back in to recover. But its still an action that shouldn't be taken lightly and should be done carefully. vs addresses and viewing keys which we want to be far easier for people to interact with. 

[51:53]- **Dodger**

One thing we should bear in mind when thinking of unified spending keys is that it's entirely possible that different keys will live in different places. If you think of the work going on to add shielded support for Zcash to Trezor & Ledger, we are likely to have the ability for hardware devices to handle transparent and Sapling keys but not Orchard.
...
in general doc-hidden things are unstable. That's a good inference. 
